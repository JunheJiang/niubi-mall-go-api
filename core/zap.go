...
import (
	...
	"niubi-mall/global"
	"niubi-mall/utils/file"
	"niubi-mall/utils/log"
	"os"
	"time"
)

func Zap() (logger *zap.Logger) {
	if ok, _ := file.PathExists(global.GVA_CONFIG.Zap.Director); !ok { // 判断是否有Director文件夹
...
func getEncoderCore(fileName string, level zapcore.LevelEnabler) (core zapcore.Core) {
	writer := log.GetWriteSyncer(fileName) // 使用file-rotatelogs进行日志分割
